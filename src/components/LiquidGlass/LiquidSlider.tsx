...
export const LiquidSlider: React.FC<LiquidSliderProps> = ({
  ...
  const MAX_STRETCH = sliderWidth * 0.1;
  const STRETCH_RESISTANCE = 0.4;

  const SCALE_REST = 0.6;
  const SCALE_DRAG = 1;
  const thumbWidthRest = THUMB_WIDTH * SCALE_REST;

  const maxDragDistance = sliderWidth - THUMB_WIDTH;
  ...
  const scaledWidth = sliderWidth * scale;
  ...
  return (
    <div
      ref={containerRef}
      style={{
        width: fillContainer ? "100%" : scaledWidth,
        height: scaledThumbHeight + (showPercentage || showMappedValue ? 60 : 0),
        position: "relative",
      }}
    ...
    >
      ...
      >
        <motion.div style={{ position: "relative", width: sliderWidth, height: THUMB_HEIGHT }}>
          <motion.div
            ref={trackRef}
            style={{
              ...
              left: 0,
              top: (THUMB_HEIGHT - SLIDER_HEIGHT) / 2,
